...
emoji.Emoji.prototype = $extend(THREE.Object3D.prototype,{
	init: function() {
		...
		while(_g1 < l) {
			var i1 = _g1++;
			var i2 = i1 * 2;
			var pos = this.getIconPos(Math.floor(Math.random() * 845));
			aOffsets[i2] = pos.x;
			aOffsets[i2 + 1] = pos.y;
		}
...
emoji.EmojiShader = function() {
	...
	this.animationFrameLength = 32;
	this._texture = new THREE.TextureLoader().load("./emo2048.png");
...
fbo.Fbo.prototype = {
	init: function(ww,hh) {
		...
		this._renderGeo = this._getParticleGeo();
		this._particles = new THREE.Points(this._renderGeo,this._renderShaderMat);
		this._line = new THREE.Line(this._renderGeo,this._renderShaderMat);
		this._mesh = new THREE.Mesh(this._renderGeo,this._renderShaderMat);
	}
...
fbo.FboMain.prototype = {
	...
	,_onAudio: function() {
		...
		this._scene.add(this._fbo.getLine());
	...
	,update: function() {
		...
		if(this._fbo != null) this._fbo.update(this._renderer);
		if(this._emoji != null) this._emoji.update();
		this._camera.radX += Math.PI / 180;
		this._camera.update();
...
fbo.RenderShaderMat = function() {
	...
	this.animationFrameLength = 32;
	var tex = THREE.ImageUtils.loadTexture("emo2048.png");
	tex.format = 1021;
	THREE.ShaderMaterial.call(this,{ uniforms : { positions : { type : "t", value : null}, pointSize : { type : "f", value : 40}, texture : { type : "t", value : tex}, scale : { type : "f", value : 3000.0}, repeat : { type : "v2", value : new THREE.Vector2(1 / this.animationFrameLength,1 / this.animationFrameLength)}}, vertexShader : this._vertex, fragmentShader : this._fragment, transparent : true, side : 2});
	this.depthTest = true;
...
fbo.SimulationShaderMat = function(ww,hh) {
	...
	THREE.ShaderMaterial.call(this,{ uniforms : { texture : { type : "t", value : texture}, timer : { type : "f", value : 0}, frequency : { type : "f", value : 0.01}, amplitude : { type : "f", value : 96}, maxDistance : { type : "f", value : 48}}, vertexShader : this._vertex, fragmentShader : this._fragment});
};
