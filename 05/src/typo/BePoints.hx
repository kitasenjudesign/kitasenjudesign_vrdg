...
class BePoints
{
	...
	public function getStrokes( points:Array<Array<Vector2>> ):Array<Stroke> {
		
		var strokes:Array<Stroke> = [];
		for (i in 0...points.length) {
			var s:Stroke = new Stroke();// points[i];
			s.init(points[i]);
			strokes.push(s);
		}
		return strokes;
	...
	public function getLetterPoints(
		...
		for (i in 0...len){
			switch (motif[i][0]){
				case "M":
					...
					_count++;
					_points[_count] = [new Vector2(_currentX, _currentY)];
				...
				case "L":
					if(isLineSplit){
						...
						_currentX = s * (motif[i][1][0] + ox);
						_currentY = s * (motif[i][1][1] + oy);
						_points[_count].push( new Vector2(_currentX, _currentY) );
		...
		public function lineTo( xx:Float, yy:Float):Void {
			...
			for (i in 0...numBunkatsu) {
				...
				var pp:Vector2 = new Vector2(_currentX + rate * dx, _currentY + rate * dy);
				//g.lineTo( pp.x, pp.y );
		...
		public function curveTo(cx:Float,cy:Float, xx:Float, yy:Float):Void {
			...
			var bezje:Be2d = new Be2d( 
				new Vector2(_currentX, _currentY),
				new Vector2(xx, yy),
				new Vector2(cx, cy)
			);
