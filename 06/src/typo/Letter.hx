...
class Letter extends Container
{
	...
	public function update():Void {
		...
		//if ( Math.abs( _old - sec ) > 55) _main.reset();
		trace(sec);
		_old = sec;
		...
		var pp:Point = _allPoints[_currentIndex];
	...
	public function clear() 
	{
		...
		if (!isColor){
			...
			shape.graphics.beginStroke(Graphics.getRGB(255,255,255, mojiAlpha));
		}
