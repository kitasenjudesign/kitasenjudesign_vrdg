...
class Sprite2D extends Sprite, implements IObject3D {
	 ...
	 */
	public function new() {super();__visible = true;__x = 0;__y = 0;__z = 0;__scaleX = 1;__scaleY = 1;__matrix = new Matrix3D();__concatenatedMatrix = new Matrix3D();__scaled = true;__render = true;__renderScaling = true;__scaling = false;initVectors();
	}

	private function initVectors():Void {__vectorIn = new Vector<Float>(3, true);__vectorOut = new Vector<Float>(3, true);
	}
	...
	private function setX(value:Float):Float {__x = value;askRendering();return __x;
	}
	...
	private function setY(value:Float):Float {__y = value;askRendering();return __y;
	}
	...
	private function setZ(value:Float):Float {__z = value;askRendering();return __z;
	}
	...
	private function setScaleX(value:Float):Float {__scaleX = value;if (__scaled) __renderScaling = true;else scaleX = __scaleX;return __scaleX;
	}
	...
	private function setScaleY(value:Float):Float {__scaleY = value;if (__scaled) __renderScaling = true;else scaleY = __scaleY;return __scaleY;
	}
	...
	private function getScaled():Bool {return __scaled;
	}

	private function setScaled(value:Bool):Bool {__scaled = value;if (__scaled) {	__renderScaling = true;} else {	__renderScaling = false;	if (__scaling) removeScaling();}return __scaled;
	}
	 ...
	 * @private
	 */
	public function askRendering():Void {__render = true;if (__scaled) __renderScaling = true;
	}
	 ...
	 * @private
	 */
	public function render(scene:Scene3D):Void {if (!__visible && visible) visible = false;else if (__visible) {	if (!visible) visible = true;	if (__render) {		var viewDistance:Float = scene._viewDistance;		InternalUtils.setMatrixPosition(__matrix, __x, __y, __z);		InternalUtils.setConcatenatedMatrix(__concatenatedMatrix, parent, __matrix);		__concatenatedMatrix.transformVectors(__vectorIn, __vectorOut);		__perspective = viewDistance / (__vectorOut[2] + viewDistance);		setPlacement();		__render = false;	}	if (__renderScaling) {		applyScaling();		__renderScaling = false;	}}
	}

	private function setPlacement():Void {x = __vectorOut[0] * __perspective;y = __vectorOut[1] * __perspective;
	}

	private function applyScaling():Void {scaleX = __scaleX * __perspective;scaleY = __scaleY * __perspective;__scaling = true;
	}

	private function removeScaling():Void {scaleX = __scaleX;scaleY = __scaleY;__scaling = false;
	}
}