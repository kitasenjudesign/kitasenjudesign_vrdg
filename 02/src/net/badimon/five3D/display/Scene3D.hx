...
class Scene3D extends Sprite {
	 ...
	 */
	public function new(autoRender:Bool = true) {super();__viewDistance = 1000;__ambientLightVector = new Vector3D(1, 1, 1);__ambientLightVectorNormalized = new Vector3D(.5773502691896258, .5773502691896258, .5773502691896258);__ambientLightIntensity = .5;if (autoRender) startRender();
	}
	...
	private function getViewDistance():Float {return __viewDistance;
	}

	private function setViewDistance(value:Float):Float {__viewDistance = value;askRendering();return __viewDistance;
	}
	...
	private function getAmbientLightVector():Vector3D {return __ambientLightVector;
	}

	private function setAmbientLightVector(value:Vector3D):Vector3D {__ambientLightVector = value;__ambientLightVectorNormalized = __ambientLightVector.clone();__ambientLightVectorNormalized.normalize();askRenderingShading();return __ambientLightVector;
	}
	...
	private function getAmbientLightVectorNormalized():Vector3D {return __ambientLightVectorNormalized;
	}
	...
	private function getAmbientLightIntensity():Float {return __ambientLightIntensity;
	}

	private function setAmbientLightIntensity(value:Float):Float {__ambientLightIntensity = value;askRenderingShading();return __ambientLightIntensity;
	}
	 ...
	 * @see #stopRender()
	 */
	public function startRender():Void {initializeRendering();
	}
	 ...
	 * @see #startRender()
	 */
	public function stopRender():Void {desinitializeRendering();
	}
	 ...
	 * @see #stopRender()
	 */
	public function forceRender():Void {render();
	}

	private function initializeRendering():Void {addEventListener(Event.ENTER_FRAME, enterFrameHandler);
	}

	private function enterFrameHandler(event:Event):Void {render();
	}

	private function render():Void {var i:UInt;var num:UInt = numChildren;for (i in 0...num) cast(getChildAt(i), IObject3D).render(this);
	}

	private function desinitializeRendering():Void {removeEventListener(Event.ENTER_FRAME, enterFrameHandler);
	}

	private function askRendering():Void {var i:UInt;var num:UInt = numChildren;for (i in 0...num) cast(getChildAt(i), IObject3D).askRendering();
	}

	private function askRenderingShading():Void {var i:UInt;var num:UInt = numChildren;for (i in 0...num) cast(getChildAt(i), IObject3D).askRenderingShading();
	}

	/**
	 * Dispose the Scene3D instance and stop the automatic rendering of the scene on every frame if necessary.
	 */
	public function dispose():Void {desinitializeRendering();
	}
}