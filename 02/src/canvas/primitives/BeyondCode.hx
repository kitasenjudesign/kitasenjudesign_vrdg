 ...
 * @author watanabe
 */
class BeyondCode extends PrimitiveBase
{
	...
	override public function init():Void {
		...
		for(i in 0...s.length){
			...
			FontTest.getLetterPoints(shape, s.substr(i, 1), true, 1, new HelveticaMedium());
			//shapes.push(shape);
			//var geo:ExtrudeGeometry = new ExtrudeGeometry(untyped shape, {amount:1});
			//var geo:ShapeGeometry = new ShapeGeometry(untyped shape, { } );
			var geo:ExtrudeGeometry = new ExtrudeGeometry(untyped shape, { amount:10,bevelEnabled:false } );
