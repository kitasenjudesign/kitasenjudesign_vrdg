...
class OCamera extends OrthographicCamera
{
    ...
    public var amp    	:Float = 400.0;
	public var radX   	:Float = 0;// Math.PI / 5;
	...
	{//super( -1000, 1000, -1000, 1000, 10, 1000);//super(fov, aspect, near, far);
	}
	
	public function init(dom:Dynamic):Void{
        _camera = this;target = new Vector3();_isMobile = OsChecker.isMobile();
        //var container:any = document.querySelector('#container');/*dom.ontouchstart	= this.onMouseDown;dom.ontouchend	= this.onMouseUp;dom.ontouchmove = this.onMouseMove;dom.onmousedown 	= this.onMouseDown;
        dom.onmouseup 		= this.onMouseUp;
        dom.onmousemove 	= this.onMouseMove;
        dom.onmousewheel 	= this.onMouseWheel;
        Browser.window.addEventListener('DOMMouseScroll',this.onMouseWheelFF);*///Browser.window.onresize = _onResize;_dom = dom;
    }
    ...
    private function onMouseDown(e):Void{
        e.preventDefault();
        this._down = true;if (_isMobile) {	var touch = e.touches[0];	if( e.touches.length<=1){		_downX = touch.pageX;		_downY = touch.pageY;	}}else{	_downX = e.clientX;	_downY = e.clientY;
        }
    ...
    private function onMouseMove(e):Void{
        ...
        //e.preventDefault();
        //console.log( "kiteru " + e.clientX +" " + e.clientY );if (_isMobile) {	var touch = e.touches[0];	if( e.touches.length<=1){		_mouseX = touch.pageX;		_mouseY = touch.pageY;	}}else{	_mouseX = e.clientX;	_mouseY = e.clientY;}    }
...
//if(!this.isActive)return;
        ...
        if(this._down) {
            ...
            this.radY = this._oRadY + dy / 100;	if (this.radY > Math.PI / 2) this.radY = Math.PI / 2;	if (this.radY < -Math.PI / 2) this.radY = -Math.PI / 2;
        }

        //console.log(this.radX + " " + this.radY + " " + x+" "+y+" "+z);
        if (_camera != null){	_updatePosition(1/4);
        }
//this.rotation.x = Math.PI / 4;
    }
	
	
	public function setFOV(fov:Float) {trace("setFOV = " + fov);//_camera.fov = fov;_camera.updateProjectionMatrix();
	}

	public function resize():Void {//_width = Browser.window.innerWidth;//_height = Browser.window.innerHeight;//_camera.aspect = _width / _height;//_camera.updateProjectionMatrix();
	}
	...
	{_dom.onmousedown 	= null;// this.onMouseDown;
        ...
        //Browser.window.addEventListener('DOMMouseScroll', this.onMouseWheelFF);Browser.window.removeEventListener('DOMMouseScroll', this.onMouseWheelFF);
	}
	
	private function _updatePosition(spd:Float=1):Void
	{var amp1		:Float = this.amp * Math.cos(this.radY);var x		:Float = target.x + amp1 * Math.sin( this.radX );//横var y		:Float = target.y + this.amp * Math.sin(this.radY);var z		:Float = target.z + amp1 * Math.cos( this.radX );//横_camera.position.x += (x - _camera.position.x) * spd;_camera.position.y += (y - _camera.position.y) * spd;_camera.position.z += (z - _camera.position.z) * spd;var t:Vector3 = target.clone();t.y += tgtOffsetY;target2 = t;
