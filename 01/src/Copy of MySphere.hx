 ...
 * @author nabe
 */
class objects.MySphere extends Object3D
{
	...
	public function update(audio:MyAudio):Void {
		...
		if (_audio!=null && _audio.isStart) {
			...
			var pp:Float = (Math.pow( _audio.freqByteData[18] / 255 , 6) ) * 0.1;
			_vr += pp;
			
			_nejireX = Math.pow(_audio.freqByteData[16] / 255, 3.5) * 0.02;
			_nejireY = Math.pow(_audio.freqByteData[18] / 255, 4.5) * Math.PI * 2 * 0.1;
			
			_noise = Math.pow(_audio.freqByteData[12] / 255, 3) * 0.1;
			_speed =  Math.pow( _audio.freqByteData[8] / 255, 4) * 0.2;
			_sphere =  Math.pow( _audio.freqByteData[4] / 255, 15);
			_noiseSpeed = 0.01 + Math.pow( _audio.freqByteData[19] / 255, 15) * 0.1;
			_scale = 1 + Math.pow(_audio.freqByteData[1] / 255, 5) * 0.2;
		...
		for (i in 0...g.vertices.length) {
			...
			g.vertices[i].y = amp * Math.sin(radY);//縦
