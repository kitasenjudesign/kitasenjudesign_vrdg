...
class ExCamera extends PerspectiveCamera
{
	...
	public function resize():Void {
		...
		_width = StageRef.stageWidth;
		_height = StageRef.stageHeight;
	...
	private function _updatePosition(spd:Float=1):Void
	{
		...
		_camera.position.x += (x+bure.cam.x - _camera.position.x) * spd;
		_camera.position.y += (y+bure.cam.y - _camera.position.y) * spd;
		_camera.position.z += (z+bure.cam.z - _camera.position.z) * spd;
