...
class MyShaderMaterial extends ShaderMaterial
{
	...
	private var ff:String = "
		...
		void main()
		{
			...
			//拡散色の決定
			vec3 diffuse = vec3(1.0,0.0,0.0) * dotNL;// + col.xyz * 0.5;
...
void main()
{
	...
	float sphere =pow(_freqByteData[4]/255.0, 5.0);
	...
	*/	
	float a = length(vv);
	float radX = -atan(vv.z, vv.x) + vv.y * sin(_count) * nejireX;//横方向の角度
	...
	*/
	...
	{
		if (_texture1 == null) {
			_texture1 = ImageUtils.loadTexture("grade.png");//mae_face.png");
		}
		...
		super({
				...
				uniforms: {
					...
					_lightPosition: { type: "v3", value: new Vector3(0,100,0)}
				}
		});
