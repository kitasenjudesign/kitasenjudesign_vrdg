...
Main3d.prototype = {
	...
	,_onLoadDAE: function() {
		var geo = this.dae.geometry;
		this._maeFaces = new faces.MaeFaces();
		this._maeFaces.init(geo);
...
faces.MaeFace = function(g) {
	...
	this._geometry = g;
	this._face = new THREE.Mesh(this._geometry,this._material);
	this._face.scale.set(10,10,10);
	this.add(this._face);
...
faces.MaeFace.prototype = $extend(THREE.Object3D.prototype,{
	...
	,addForce: function(f) {
		if(!this.enabled) {
			...
			this._life = 0;
			this.enabled = true;
			this._vx += 0.3 * (Math.random() - 0.5);
			this._vy += 0.3 * (Math.random() - 0.5);
			this._vz += 0.3 * (Math.random() - 0.5);
		}
	}
	,update: function(audio) {
		if(this._life++ > 15) this.enabled = false;
		this._calcLifeRatio();
		this._face.rotation.y += this._vy;
		this._vx *= 0.97;
		this._vy *= 0.97;
		this._vz *= 0.97;
		this._material.update(audio,this._lifeRatio);
		this._gauge.update(audio,this._lifeRatio);
	}
	,setMaterial: function(type) {
		this._material.setWireframe(type == 0);
	}
...
faces.MaeLines.prototype = $extend(THREE.Object3D.prototype,{
	...
	,update: function(audio) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			if(audio.freqByteDataAry[this._faces[i].randomIndex[0]] > 15) {
				this._faces[i].addForce(1);
				this._connectLine(this._faces[i].position,new THREE.Vector3(100 * (Math.random() - 0.5),-100,0),Math.random());
			}
		}
	}
	,_resetLine: function() {
...
faces.MaePlate.prototype = $extend(THREE.Object3D.prototype,{
	init: function(ww) {
		...
		this._stage = this._createStage(canvas,256,256);
		this._material = this._getMaterial(canvas);
		this._plane = new THREE.Mesh(new THREE.PlaneGeometry(ww,ww,1,1),this._material);
		this._plane.position.x += ww / 2;
		this.add(this._plane);
		this._textCh = new objects.SpacingText("CH",objects.SpacingText.getFont(42,"700","Roboto Condensed"),0.5,"#ffffff");
		this._textCh.y = 37;
		this._stage.addChild(this._textCh);
		this._textNo = new objects.SpacingText("0001",objects.SpacingText.getFont(80,"700","Roboto Condensed"),0.5,"#ffffff");
		this._stage.addChild(this._textNo);
		this._textNo.x = 60;
		this._textNo.y = 4;
		this._textTime = new objects.SpacingText("10:20:23",objects.SpacingText.getFont(60,"700","Roboto Condensed"),1,"#ffffff");
		this._textTime.y = 85;
		this._stage.addChild(this._textTime);
		this._stage.update();
		this._material.map.needsUpdate = true;
		var g = new THREE.Geometry();
		g.vertices.push(new THREE.Vector3(0,-50,0));
		g.vertices.push(new THREE.Vector3(0,50,0));
		this.updateText();
	}
	,updateText: function() {
...
faces.MaeShaderMaterial = function() {
	...
	THREE.ShaderMaterial.call(this,{ vertexShader : this.vv, fragmentShader : this.ff, uniforms : { texture : { type : "t", value : faces.MaeShaderMaterial._texture1}, _noise : { type : "f", value : 1.5 + Math.random()}, _freqByteData : { type : "fv1", value : sound.MyAudio.a.freqByteDataAry}, _count : { type : "f", value : 100 * Math.random()}, _lightPosition : { type : "v3", value : new THREE.Vector3(0,100,50)}, _wireframe : { type : "f", value : 1}}});
	this.wireframe = true;
};
...
faces.MaeShaderMaterial.prototype = $extend(THREE.ShaderMaterial.prototype,{
	...
	,_updateFreq: function(audio,lifeRatio) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			this._freq[i] = audio.freqByteData[this._indecies[i]];
		}
	}
});
var haxe = {};
...
faces.MaeFace.R_MODE_A = 0;
faces.MaeFace.R_MODE_B = 1;
faces.MaeFace.R_MODE_C = 2;
faces.MaeFaces.P3 = 3;
faces.MaeFaces.P2 = 2;
faces.MaeFaces.P1 = 1;
