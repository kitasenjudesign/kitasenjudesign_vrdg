...
Main3d.prototype = {
	...
	,_run: function() {
		...
		if(this._maeFaces != null && this._audio != null) this._maeFaces.update(this._audio,this.camera);
...
faces.MaeFaceMesh.prototype = $extend(THREE.Mesh.prototype,{
	...
	,setRotMode: function(n) {
		...
		this._vy = 0;
		this._vz = 0;
		if(Math.random() < 0.5) this._isForceRandom = true; else this._isForceRandom = false;
	}
	,addForce: function(idx,f) {
		if(!this._isForceRandom) {
			if(idx == 0) this._vx += f * 0.5 * this._d.x;
			if(idx == 1) this._vy += f * 0.5 * this._d.y;
			if(idx == 2) this._vz += f * 0.5 * this._d.z;
		} else {
			if(idx == 0) this._vx += Math.random() - 0.5;
			if(idx == 1) this._vy += Math.random() - 0.5;
			if(idx == 2) this._vz += Math.random() - 0.5;
		}
	}
...
faces.MaeFaces.prototype = $extend(THREE.Object3D.prototype,{
	...
	,update: function(audio,camera) {
		...
		this._lines.update(audio,camera);
	}
});
...
faces.MaeShaderMaterial = function() {
	...
	if(faces.MaeShaderMaterial._texture1 == null) faces.MaeShaderMaterial._texture1 = THREE.ImageUtils.loadTexture("../../assets/" + "face/dede_face_diff.png");
	this._indecies = [];
	this._freq = [];
	var _g1 = 0;
...
faces.MaeShaderMaterial.prototype = $extend(THREE.ShaderMaterial.prototype,{
	...
	,update: function(a,lifeRatio) {
		if(a != null && a.isStart) {
			...
			this.uniforms._colLoop.value = 0.01 + Math.pow(a.freqByteData[this._indecies[3]] / 255,1.5) * 100;
			this.uniforms._counter.value += 0.01;
		}
	}
